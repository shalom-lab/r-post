#!/usr/bin/env node
/**
 * 选题工作流合一：ideate（刷候选）| outline（写大纲）
 * 共用 prompt-rules 的 topic_prompt_*.md。
 *
 *   node scripts/topic-work.mjs --mode ideate [--quota 5] [--promptId default]
 *   node scripts/topic-work.mjs --mode outline [--id xxx | --scheduled] [--promptId default]
 */
...
async function runIdeate(args, store, system) {
  ...
  console.log(`选题（含大纲）… quota=${quota}`);
  ...
  console.log(`追加 ${added.length} 条（其中 ${withOutline} 条已带大纲）`);
}
...
async function runOutline(args, store, system) {
  ...
  let targets = [];
  if (args.id || process.env.TOPIC_ID) {
    const id = (args.id || process.env.TOPIC_ID).trim();
    ...
    throw new Error("outline 模式需要 --id 或 --scheduled");
  }

  if (!targets.length) {
    console.log("没有需要大纲的选题");
    return;
  }
  ...
  for (const item of targets) {
    ...
    console.log(`已大纲: ${item.id} → ${outline.workingTitle || item.title}`);
  }
}
...
async function main() {
  ...
  console.log(`选题 Prompt: ${promptPath} · mode=${mode}`);
